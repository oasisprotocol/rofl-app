...
describe('ResourcesCpu', () => {
  ...
  it('should handle very large CPU counts', () => {
    const { container: _container } = render(<ResourcesCpu value={128} />)
...
describe('ResourcesMemory', () => {
  it('should return N/A for null value', () => {
    const { container: _container } = render(<ResourcesMemory value={null} />)
    expect(screen.getByText('N/A')).toBeInTheDocument()
  })

  it('should return N/A for undefined value', () => {
    const { container: _container } = render(<ResourcesMemory value={undefined} />)
    expect(screen.getByText('N/A')).toBeInTheDocument()
  })

  it('should return N/A for 0 value', () => {
    const { container: _container } = render(<ResourcesMemory value={0} />)
    expect(screen.getByText('N/A')).toBeInTheDocument()
  })

  it('should convert MB to GB correctly', () => {
    const { container: _container } = render(<ResourcesMemory value={1024} />)
    expect(screen.getByText('1 GB')).toBeInTheDocument()
  })

  it('should handle 2048 MB', () => {
    const { container: _container } = render(<ResourcesMemory value={2048} />)
    expect(screen.getByText('2 GB')).toBeInTheDocument()
  })

  it('should handle 4096 MB', () => {
    const { container: _container } = render(<ResourcesMemory value={4096} />)
    expect(screen.getByText('4 GB')).toBeInTheDocument()
  })

  it('should handle 8192 MB', () => {
    const { container: _container } = render(<ResourcesMemory value={8192} />)
    expect(screen.getByText('8 GB')).toBeInTheDocument()
  })

  it('should handle 16384 MB', () => {
    const { container: _container } = render(<ResourcesMemory value={16384} />)
    expect(screen.getByText('16 GB')).toBeInTheDocument()
  })

  it('should handle fractional GB values', () => {
    const { container: _container } = render(<ResourcesMemory value={1536} />)
    expect(screen.getByText('1.5 GB')).toBeInTheDocument()
  })

  it('should handle 512 MB', () => {
    const { container: _container } = render(<ResourcesMemory value={512} />)
    expect(screen.getByText('0.5 GB')).toBeInTheDocument()
  })

  it('should handle 256 MB', () => {
    const { container: _container } = render(<ResourcesMemory value={256} />)
    expect(screen.getByText('0.3 GB')).toBeInTheDocument()
  })

  it('should handle large memory values', () => {
    const { container: _container } = render(<ResourcesMemory value={1048576} />)
...
describe('ResourcesStorage', () => {
  it('should return N/A for null value', () => {
    const { container: _container } = render(<ResourcesStorage value={null} />)
    expect(screen.getByText('N/A')).toBeInTheDocument()
  })

  it('should return N/A for undefined value', () => {
    const { container: _container } = render(<ResourcesStorage value={undefined} />)
    expect(screen.getByText('N/A')).toBeInTheDocument()
  })

  it('should return N/A for 0 value', () => {
    const { container: _container } = render(<ResourcesStorage value={0} />)
    expect(screen.getByText('N/A')).toBeInTheDocument()
  })

  it('should convert MB to GB correctly', () => {
    const { container: _container } = render(<ResourcesStorage value={1024} />)
    expect(screen.getByText('1 GB Storage')).toBeInTheDocument()
  })

  it('should handle 2048 MB', () => {
    const { container: _container } = render(<ResourcesStorage value={2048} />)
    expect(screen.getByText('2 GB Storage')).toBeInTheDocument()
  })

  it('should handle 10240 MB (10 GB)', () => {
    const { container: _container } = render(<ResourcesStorage value={10240} />)
    expect(screen.getByText('10 GB Storage')).toBeInTheDocument()
  })

  it('should handle 51200 MB (50 GB)', () => {
    const { container: _container } = render(<ResourcesStorage value={51200} />)
    expect(screen.getByText('50 GB Storage')).toBeInTheDocument()
  })

  it('should handle 102400 MB (100 GB)', () => {
    const { container: _container } = render(<ResourcesStorage value={102400} />)
    expect(screen.getByText('100 GB Storage')).toBeInTheDocument()
  })

  it('should round to nearest GB', () => {
    const { container: _container } = render(<ResourcesStorage value={1536} />)
    expect(screen.getByText('2 GB Storage')).toBeInTheDocument()
  })

  it('should round up correctly', () => {
    const { container: _container } = render(<ResourcesStorage value={2000} />)
    expect(screen.getByText('2 GB Storage')).toBeInTheDocument()
  })

  it('should handle large storage values', () => {
    const { container: _container } = render(<ResourcesStorage value={1024000} />)
