...
export const metadataFormSchema = z.object({
  ...
  author: z.literal('').or(
    z.string().regex(/^([a-zA-Z\s]+)\s+<([^@\s]+@[^@\s.]+(?:\.[^@\s.]+)*\.[a-zA-Z]{2,})>$/, {
      message:
        'Author must follow the format "Name <valid_email>". The name can only contain letters and spaces, and the email address must be in angle brackets.',
    }),
  ),
